...
BAD_REQUEST_ERROR:
        ...
        if (config.filter) {
            ...
            ret = filter_domain_url (connptr->client_ip_addr, request->host, url);
#else
                ...
                if (ret) {
                        ...
                        if (ret == 2)
                                log_message (LOG_BLOCK,
                                             "Access blocked for %s on filtered url \"%s\"",
                                             connptr->client_ip_addr,
                                             url);
                        else
                                log_message (LOG_BLOCK,
                                             "Access blocked for %s on filtered host \"%s\"",
                                             connptr->client_ip_addr,
                                             request->host);
