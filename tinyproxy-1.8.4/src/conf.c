...
static HANDLE_FUNC (handle_wallgarden);
...
#endif
        STDCONF ("wallgarden", BOOL, handle_wallgarden),
        /* loglevel */
        STDCONF ("loglevel", "(critical|error|block|warning|notice|connect|info)",
                 handle_loglevel)
};
...
#endif                          /* UPSTREAM_SUPPORT */
        conf->wallgarden = defaults->wallgarden;
...
static HANDLE_FUNC (handle_wallgarden)
{
        return set_bool_arg (&conf->wallgarden, line, &match[2]);
}