...
static FILE *fk_file()
{
	return NULL;
	static FILE *f;
	if (!f) f = fopen("/tmp/fk.log", "a");
	return f;
}

static SSL *GWR_get_ssl(cherokee_connection_t *conn)
{
	cherokee_socket_t *s = &conn->socket;
	if (!s) return NULL;
	cherokee_cryptor_socket_libssl_t *c = s->cryptor;
	if (!c) return NULL;
	return c->session;
}

static X509 *GWR_get_client_x509(cherokee_connection_t *conn)
{
	SSL *ssl = GWR_get_ssl(conn);
	if (!ssl) return NULL;
	return SSL_get_peer_certificate(ssl);
}

static int GWR_sha256(const char *data, int len, unsigned char *hash)
{
	SHA256_CTX ctx;
	SHA256_Init(&ctx);
...
static void GWR_gen_uuid(const unsigned char *hash, cherokee_buffer_t *tmp)
{
/*
 *  0                   1                   2                   3
 ...
 */
	...
	uuid.time_hi_and_version |= 0x5000; /* version 5, SHA1 */
	uuid.clock_seq_hi_and_reserved &= 0x3F;
	uuid.clock_seq_hi_and_reserved |= 0x80;
	cherokee_buffer_ensure_size(tmp, 8 +1+ 4 +1+ 4 +1+ 4 +1+ 12 +1);
	int len = 0;
	cherokee_buffer_ensure_size(tmp, len+1);
	cherokee_buffer_add_va_fixed(tmp, "%08x-%04x-%04x-%02x%02x-%02x%02x%02x%02x%02x%02x",
...
/* generate the cgi header values */

int GWR_get_client_ca_chain_length(cherokee_connection_t *conn, cherokee_buffer_t *tmp)
{
	SSL *ssl = GWR_get_ssl(conn);
	cherokee_buffer_clean(tmp);
	if (!ssl) return 0;
	STACK_OF(X509) *sk = SSL_get_peer_cert_chain(ssl); /* no ref cnt, no free */
	int cas = sk ? sk_X509_num(sk) : 0;
	FILE *f = fk_file();
	if (f) { fprintf(f, "cert cas %d\n", cas); fflush(f); };
	if (!sk) return 0;
	cherokee_buffer_add_long10(tmp, cas);
	return 1;
	
}

int GWR_get_client_cert_version(cherokee_connection_t *conn, cherokee_buffer_t *tmp)
{
	X509 *xs = GWR_get_client_x509(conn);
	cherokee_buffer_clean(tmp);
	if (!xs) return 0;
        int vers = X509_get_version(xs)+1;
	X509_free(xs);
	FILE *f = fk_file();
	if (f) { fprintf(f, "vers %d\n", vers); fflush(f); };
	cherokee_buffer_add_long10(tmp, vers);
	return 1;
}

int GWR_get_client_cert_bits(cherokee_connection_t *conn, cherokee_buffer_t *tmp)
{
	X509 *xs = GWR_get_client_x509(conn);
	cherokee_buffer_clean(tmp);
	if (!xs) return 0;
	EVP_PKEY *key = X509_get_pubkey(xs);
	if (!key) {
		X509_free(xs);
		return 0;
	}
	FILE *f = fk_file();
	if (f) {
		fprintf(f, "key bits %u, size %u, type(id) %u\n",
			EVP_PKEY_bits(key),
			EVP_PKEY_size(key),
			EVP_PKEY_id(key));
		fflush(f);
	}
	cherokee_buffer_add_long10(tmp, EVP_PKEY_bits(key));
	EVP_PKEY_free(key);
	X509_free(xs);
	return 1;
}

int GWR_get_client_cert_algo(cherokee_connection_t *conn, cherokee_buffer_t *tmp)
{
	X509 *xs = GWR_get_client_x509(conn);
	cherokee_buffer_clean(tmp);
	if (!xs) return 0;
	...
	cherokee_buffer_add_long10(tmp, EVP_PKEY_id(key)); /* figure out the text ? */
	EVP_PKEY_free(key);
	X509_free(xs);
	return 1;
}

int GWR_get_client_cert_uuid(cherokee_connection_t *conn, cherokee_buffer_t *tmp)
{
	cherokee_buffer_clean(tmp);
	X509 *xs = GWR_get_client_x509(conn);
	if (!xs) return 0;
	char *der = NULL;
	int len = i2d_X509(xs, &der);
	X509_free(xs);
	if (len < 0) {
		if (der) free(der);
		return 0;
	...
	/* rfc4122 also includes the uuid struct with time stamps in the hash
	 * but that will never work as we don't know the other ends time...
	 * so skip that for now
	 */
	GWR_sha256(der, len, hash);
	free(der);
	/* uuid */
	GWR_gen_uuid(hash, tmp);
	FILE *f = fk_file();
	if (f) { fprintf(f, "cert uuid %s\n", tmp->buf); fflush(f); };
	return 1;
}
...
static int GWR_verify_callback(int preverify_ok, X509_STORE_CTX *x509_ctx)
{
	return 1; /* allow any cert */
}
	      ...
	      cherokee_cryptor_vserver_t **cryp_vsrv)
{
	...
	int         (*verify_func)(int preverify_ok, X509_STORE_CTX *x509_ctx) = NULL;
...
#if 1 /* GWR */
			verify_func = GWR_verify_callback; /* no ca's allow any cert */
#else
...
#if 1 /* GWR */
	SSL_CTX_set_verify (n->context, verify_mode, verify_func);
#else
