...
#if defined CONFIG_ENV_OFFSET1 && defined CONFIG_ENV_OFFSET2
/**
 * Is we accessing u-boot env 0 or 1.
 * For use with two u-boot
 */
static int get_env_type(void)
{
	int pageSize = 0;
	...
	offset = GLOBAL_SOFTWARE2 % pageSize;
	map = mmap((caddr_t)0, pageSize, PROT_READ, MAP_SHARED, fd, GLOBAL_SOFTWARE2-offset);
	if (MAP_FAILED==map) {
		fprintf (stderr,"mmap failed");
		close(fd);
		return -1;
	}
	regValue = *((int*)(map+offset));
	munmap(map,pageSize);
	close (fd);
	return regValue & 1;
}
...
#else
#if defined CONFIG_ENV_OFFSET1 && defined CONFIG_ENV_OFFSET2
	DEVNAME (0) = env ? DEVICE1_NAME2 : DEVICE1_NAME;
#else
...
#ifdef HAVE_REDUND
#if defined CONFIG_ENV_OFFSET1 && defined CONFIG_ENV_OFFSET2
	DEVNAME (1) = env ? DEVICE2_NAME2 : DEVICE2_NAME;
#else
