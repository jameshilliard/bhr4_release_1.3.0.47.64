...
extern int GWR_get_client_cert_uuid(cherokee_connection_t *conn, cherokee_buffer_t *tmp);
extern int GWR_get_client_cert_version(cherokee_connection_t *conn, cherokee_buffer_t *tmp);
extern int GWR_get_client_cert_bits(cherokee_connection_t *conn, cherokee_buffer_t *tmp);
extern int GWR_get_client_cert_algo(cherokee_connection_t *conn, cherokee_buffer_t *tmp);
extern int GWR_get_client_ca_chain_length(cherokee_connection_t *conn, cherokee_buffer_t *tmp);
...
#if 1 /* GWR */
		if (CONN_VSRV(conn)->ssl_env) {
			if (GWR_get_client_cert_version(conn, tmp))
				set_env (cgi, "SSL_CLIENT_CERT_VERSION", tmp->buf, tmp->len);
			if (GWR_get_client_cert_bits(conn, tmp))
				set_env (cgi, "SSL_CLIENT_CERT_BITS", tmp->buf, tmp->len);
			if (GWR_get_client_cert_algo(conn, tmp))
				set_env (cgi, "SSL_CLIENT_CERT_ALGORITHM", tmp->buf, tmp->len);
			if (GWR_get_client_cert_uuid(conn, tmp))
				set_env (cgi, "SSL_CLIENT_CERT_UUID", tmp->buf, tmp->len);
			if (GWR_get_client_ca_chain_length(conn, tmp))
				set_env (cgi, "SSL_CLIENT_CA_LENGTH", tmp->buf, tmp->len);
		}
#endif
	} else  {
