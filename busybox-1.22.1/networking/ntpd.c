...
step_time(double offset)
{
	...
	time_t tval;
	time_t told;
	char old_t_buf[sizeof("yyyy-mm-dd hh:mm:ss") + /*paranoia:*/ 4] = {0};
	int fd;
	...
	if (fd == -1)
		bb_perror_msg_and_die("/dev/rtc0");
	gettimeofday(&tvc, NULL); /* never fails */
	dtime = tvc.tv_sec + (1.0e-6 * tvc.tv_usec) + offset;
	d_to_tv(dtime, &tvn);
	if (gmtime_r(&tvn.tv_sec, &tmtime) == 0)
		bb_perror_msg_and_die("gmtime_r");
	rtime.tm_sec = tmtime.tm_sec;
	rtime.tm_min = tmtime.tm_min;
	rtime.tm_hour = tmtime.tm_hour;
	rtime.tm_mday = tmtime.tm_mday;
	rtime.tm_mon = tmtime.tm_mon;
	rtime.tm_year = tmtime.tm_year;
	if (settimeofday(&tvn, NULL) == -1)
		bb_perror_msg_and_die("settimeofday");
	if (ioctl(fd, RTC_SET_TIME, &rtime) == -1)
		bb_perror_msg_and_die("set RTC");
	if (close(fd))
		bb_error_msg("error closing RTC file");
	VERB2 {
		tval = tvc.tv_sec;
	...
	told = tvc.tv_sec;	
	strftime(old_t_buf, sizeof(old_t_buf), "%Y-%m-%d %H:%M:%S", localtime(&told));

	openlog ("ntpd", LOG_PERROR | LOG_PID , LOG_LOCAL1);
	syslog (LOG_INFO, "System clock updated by ntpd from %s to %s", old_t_buf, buf);
	closelog ();
...
// we end up here. Looks like _our_ clock is off.
		...
		if (G.status_fd != -1) {
			(void)write(G.status_fd, msg_buf, 1);
		}
...
//		G.time_was_stepped = 1;
	...
	G.status_fd = open("/tmp/ntpd.status", O_WRONLY | O_NONBLOCK);
}
...
#undef G
	...
	while (!bb_got_signal && G.peer_cnt != 0) {
		llist_t *item;
		unsigned i, j;
		int nfds, timeout;
		double nextaction;
